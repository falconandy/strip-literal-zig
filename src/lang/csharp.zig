...
pub fn factories() []const base.Factory {
    const string_defintions = .{
        ...
        types.StringDefinition{ .prefixes = &.{"$\""}, .postfix = "\"", .skip = &.{ "\\\"", "\\\\", "{{" }, .template_prefix = "{", .template_postfix = "}" },
        types.StringDefinition{ .prefixes = &.{ "$@\"", "@$\"" }, .postfix = "\"", .skip = &.{ "\"\"", "{{" }, .multiline = true, .template_prefix = "{", .template_postfix = "}" },
    };
